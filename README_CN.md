...
> **⚠️ 测试版本警告 (Beta)**
> 本次 V2 更新引入了重大的架构变更（数据持久化）。虽然已在作者博客上测试通过，但**尚未得到广泛的用户验证**。
> 请务必在升级前**备份您的博客数据**，并谨慎使用。如果您遇到任何问题，欢迎提交 Issue 反馈。
...
3.  **运行迁移工具**：
    ...
    *   (可选) 可以在脚本中配置 `REMOVE_UPDATED_FIELD = true` 来自动清理 MD 文件（默认关闭）。
    ...
    > **⚠️ 重要提示：热力图数据可能部分丢失**
    > 由于旧版插件可能未保存所有历史数据，迁移过程中重建的热力图可能无法 100% 还原您过去的所有更新频次（仅能基于现有文件时间重建）。
    > **但是，鉴于 V1.0.0 版本存在原因未明的严重历史数据丢失风险，我们强烈建议所有用户立即升级到 V2 版本以启用数据持久化保护。**
